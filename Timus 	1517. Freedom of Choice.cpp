...
int suffix_array_construct()
{
    ...
    for(int i=0;i<n-1;i++)
    {
        if(suffixarray[i]<exact_pos && suffixarray[i+1]>exact_pos || (suffixarray[i]>exact_pos && suffixarray[i+1]<exact_pos)) if(ans==lcp[i]) {pos=suffixarray[i];break;}
    }
    res=s.substr(pos,ans);
    return ans;
}
