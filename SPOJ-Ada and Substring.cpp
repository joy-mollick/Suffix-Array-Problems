...
/// You have to understand the construction of suffix array very well ,then it is very simple 
...
void suffix_array_construct()
{
    ...
    long long len=n;
    long long arr[26]={0};
    long long nmber_of_dstnct=len-suffixarray[0];
    arr[s[suffixarray[0]]-'a']+=nmber_of_dstnct;
    for(int i=1;i<n;i++)
    {
        nmber_of_dstnct=(len-(long long)suffixarray[i])-(long long)lcp[i-1];
        arr[s[suffixarray[i]]-'a']+=nmber_of_dstnct;
    }
    cout<<arr[0];
    for(int i=1;i<26;i++) cout<<" "<<arr[i];
}
