...
/// Normal distinct substring calculation 
...

{
    ...
    vector<int>lcp=lcp_construct(arr,n);
    int num_of_duplicate_substrings=0;
    for(int i=0;i<n;i++) num_of_duplicate_substrings+=lcp[i];
    int total_substrings=n*(n+1)/2;
    int ans=total_substrings-num_of_duplicate_substrings;
    cout<<ans<<'\n';
    }
}
